...
p2 <- crime.total %>%
  ...
  labs(y = "Offences per thousand inhabitants",
       x = "Countries", title = "Crime Data for European Countries {closest_state}") +
  transition_states(TIME, transition_length = 1, state_length = 10) +
  ease_aes("sine-in-out")

animate(p2,  width = 600, height = 500, end_pause = 50, renderer = gifski_renderer("gganim2.gif"))
```
...
ggplot(crime.ireland2,aes(crime.ireland2$TIME,crime.ireland2$Values)) + 
  geom_line() + 
  geom_point() + 
  facet_wrap(~crime.ireland2$ICCS,scales = "free") +
...
final <- cbind(imig10yr,crime.ireland)
final <- final[,c(-2:-6,-8:-11)]
cor(final[,3:12],final$`Net migration`)
